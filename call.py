...
def vidhevc():
    ...
    if(cmd == ""): #single
        ...
        os.system(f"ffmpeg -hwaccel auto -i \"{url}\" -map 0:v -map 0:a? -map 0:s? -c:v libx265 -max_muxing_queue_size 9999 -rc constqp -qp 24 -b:v 0K -c:a libopus -b:a 190k -c:s copy \"{os.path.splitext(url)[0]+append}\"")
        print("\a")
    elif(cmd == "1"): #numbered
        ...
        for numb in range(numb, numb_last+1):
            ...
            for i in episodes:
                if str(numb_final) in i:
                    ...
                    os.system(f"ffmpeg -hwaccel auto -i \"{i}\" -map 0:v -map 0:a? -map 0:s? -c:v libx265 -max_muxing_queue_size 9999 -rc constqp -qp 24 -b:v 0K -c:a libopus -b:a 190k -c:s copy \"{finali}\"")
            print("\a")
    else:
        clear()
        name()
